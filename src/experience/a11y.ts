 ...
 */
export function evidenceOf(node: AxeNode, measured?: ElementBox): ElementEvidence | undefined {
  const selector = selectorOf(node)
  if (selector === undefined) return undefined
  const text = textOf(node)
  const element: ElementRef = { tag: tagOf(node, selector), selector, ...(text === undefined ? {} : { text }) }
  // axe itself reports no geometry, so the rectangle is measured separately in
  // the page. An unmeasured node keeps a zero box, which states that the
  // rectangle is unknown rather than claiming the element sits at the origin.
  const box: ElementBox = measured ?? { x: 0, y: 0, width: 0, height: 0, space: 'viewport' }
  return { element, box }
}
...
export async function checkAccessibility(page: Page): Promise<readonly VisualViolation[]> {
  ...
  // One measurement pass for every target axe reported, so a finding can be
  // pointed at and captured rather than only named.
  const selectors = report.violations.flatMap(violation => violation.nodes.map(node => selectorOf(node))).filter((selector): selector is string => selector !== undefined)
  const measured = await page.evaluate((wanted: readonly string[]) => wanted.map((selector) => {
    try {
      const element = document.querySelector(selector)
      if (element === null) return null
      const rect = element.getBoundingClientRect()
      if (rect.width === 0 && rect.height === 0) return null
      return { x: Math.round(rect.x), y: Math.round(rect.y), width: Math.round(rect.width), height: Math.round(rect.height), space: 'viewport' as const }
    } catch {
      // A selector axe produced may not be valid CSS; an unmeasured node is
      // reported as unmeasured rather than failing the whole scan.
      return null
    }
  }), selectors)
  const boxBySelector = new Map<string, ElementBox>()
  selectors.forEach((selector, index) => { const box = measured[index]; if (box !== null && box !== undefined) boxBySelector.set(selector, box) })
  return report.violations.map((violation) => {
    const evidence = violation.nodes
      .slice(0, MAX_AXE_NODES)
      .map(node => { const selector = selectorOf(node); return evidenceOf(node, selector === undefined ? undefined : boxBySelector.get(selector)) })
