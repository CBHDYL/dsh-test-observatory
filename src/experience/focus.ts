 ...
 */
export function detectContentRegion(): ContentRegion | undefined {
  ...
  const textLength = (element: Element): number => (element.textContent ?? '').replace(/\s+/gu, ' ').trim().length
  ...
  let best: ContentRegion | undefined
  let bestScore = 0
  let bestTag = ''
  for (const element of candidates) {
    if (!visible(element)) continue
    const chars = textLength(element)
    if (chars < bodyChars * 0.3) continue
    const rect = element.getBoundingClientRect()
    if (rect.width < 120 || rect.height < 120) continue
    if (rect.width * rect.height > viewportArea * 0.85) continue
    ...
    if (score > bestScore) {
      ...
      let inkArea = 0
      const seenRects: { left: number; top: number; right: number; bottom: number }[] = []
      for (const child of Array.from(element.querySelectorAll('*'))) {
        ...
        for (const other of seenRects) {
          if (other.left <= box.left && other.top <= box.top && other.right >= box.right && other.bottom >= box.bottom) { covered = true; break }
        }
        if (covered) continue
        seenRects.push(box)
 ...
 */
export async function focusRegion(page: Page): Promise<ContentRegion | undefined> {
  await ensurePageHelpers(page)
  const evaluator = page as unknown as {
    evaluate: (fn: () => ContentRegion | undefined) => Promise<ContentRegion | undefined>
  }
