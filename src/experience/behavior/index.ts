 ...
 */
export function resolveBehavior(declared: string | BehaviorOverride | undefined): PersonaBehavior {
  if (declared === undefined) return DEFAULT_BEHAVIOR
 ...
 */
export function behaviorDimensions(behavior: PersonaBehavior): readonly string[] {
  const labels: string[] = []
  ...
  if (behavior.timing.hesitateMs !== undefined) labels.push('hesitant')
  if (behavior.timing.paceMs !== undefined) labels.push('paced')
  if (behavior.environment.network !== undefined) labels.push(behavior.environment.network)
  if (behavior.environment.cpuThrottle !== undefined) labels.push('cpu×' + String(behavior.environment.cpuThrottle))
