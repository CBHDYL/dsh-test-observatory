...
export const BEHAVIOR_PRESETS: Readonly<Record<string, PersonaBehavior>> = {
  neutral: NEUTRAL,
  ...
  //
  // `slow4g` rather than `slow3g`: the throttle only shapes bandwidth, so a
  // mobile budget is realistic here without the several-hundred-millisecond
  // round trip a congested cell network adds, which a locally served app never
  // sees. A caller who wants the harsher profile can override it, and the
  // settle budget below accommodates it.
  mobile: {
    id: 'mobile',
    summary: 'Runs on a constrained connection and must cope with being interrupted.',
    input: { boundaryInputs: [], doubleSubmit: false },
    timing: { settleBudgetMs: 45_000, waitForIdle: true, paceMs: 150 },
    modality: { pointer: true, tabBudget: 50 },
    environment: { network: 'slow4g', cpuThrottle: 4 },
    recovery: { retries: 1, alternativePaths: ['reload'] },
  },
...
export const BEHAVIOR_PRESET_IDS: readonly string[] = Object.keys(BEHAVIOR_PRESETS)
 ...
 */
export function presetById(id: string): PersonaBehavior | undefined {
