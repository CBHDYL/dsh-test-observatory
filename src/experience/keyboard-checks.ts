 ...
 */
export async function checkKeyboard(page: Page): Promise<readonly VisualViolation[]> {
  ...
  const inspect = (config: { reachable: string; maxSamples: number; textLimit: number; drawsFocusSource: string }): VisualViolation[] => {
    ...
    const elementInfo = (element: Element): ElementEvidence => {
      ...
      const box: ElementBox = { x: Math.round(rect.left), y: Math.round(rect.top), width: Math.round(rect.width), height: Math.round(rect.height), space: 'viewport' }
    ...
    const withoutIndicator: Element[] = []
 ...
 * @returns the violations found, in rule order.
 */
export async function probeOpenDialog(page: Page): Promise<readonly VisualViolation[]> {
  const found: VisualViolation[] = []
  ...
  for (let index = 0; index < TRAP_PROBE_TABS; index++) await page.keyboard.press('Tab')
  const escaped = await dialog.evaluate((node: Element): boolean => {
