 ...
 */
function revParse(cwd: string, args: readonly string[]): Promise<string> {
  return new Promise<string>((resolve) => {
    execFile(
      'git',
      ['-C', cwd, ...args],
      { timeout: REVISION_TIMEOUT_MS, maxBuffer: 64 * 1024 },
      (error, stdout) => {
 ...
 */
export async function detectRevision(cwd: string): Promise<Revision> {
