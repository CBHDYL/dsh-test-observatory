...
import type { CheckFinding, ReportModel } from '../report/types.ts'
 ...
 */
export function llmNarrativeWriter(llm: NarrativeLlm, route: { readonly provider: string; readonly model: string }): NarrativeWriter {
 ...
 */
export function buildNarrativePrompt(model: ReportModel): string {
 ...
 */
export function parseRunNarrative(text: string): RunNarrative {
  ...
  if (rawFindings !== undefined && !Array.isArray(rawFindings)) throw new Error('narrative findings must be an array when present')
  const findings: FindingNarrative[] = []
 ...
 */
export function applyNarrative(model: ReportModel, narrative: RunNarrative): ReportModel {
  const byRule = new Map(narrative.findings.map(finding => [finding.rule, finding]))
  const checks: CheckFinding[] = (model.checks ?? []).map((check) => {
