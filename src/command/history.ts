...
interface Snapshot { runId: string; runAt: string; score: number; durationSeconds: number; tests: readonly Pick<ReportTest, 'name' | 'path' | 'status'>[] }
interface History { version: 1; runs: readonly Snapshot[] }
export interface HistoryProjection { trend: readonly TrendPoint[]; regressions: readonly Regression[]; recovered: readonly RecoveredTest[]; tests: readonly ReportTest[] }
...
export async function projectHistory(path: string, model: ReportModel): Promise<HistoryProjection> {
  ...
  try { history = JSON.parse(await readFile(path, 'utf8')) as History } catch (error: unknown) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error }
  ...
  const tests = model.tests.map(test => {
    ...
    if (old?.status === 'failed' && test.status === 'passed') recovered.push({ name: test.name, evidence: 'Passed after failing in ' + previous?.runId })
    ...
    return flaky ? { ...test, status: 'flaky' as const } : test
  })
  const snapshot: Snapshot = { runId: model.meta.runId, runAt: model.meta.runAt, score: model.verdict.score, durationSeconds: model.summary.durationSeconds, tests: model.tests.map(({ name, path: testPath, status }) => ({ name, path: testPath, status })) }
