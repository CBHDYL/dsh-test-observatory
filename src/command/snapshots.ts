 ...
 */
export function readSnapshotCounts(value: unknown): SnapshotCounts | undefined {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return undefined
  const raw = (value as { snapshot?: unknown }).snapshot
 ...
 */
export function hasUnreviewedBaselines(counts: SnapshotCounts): boolean {
 ...
 */
export function describeSnapshots(counts: SnapshotCounts | undefined): string | undefined {
  if (counts === undefined || counts.total === 0) return undefined
  const parts: string[] = []
