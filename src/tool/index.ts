...
import type { TestCaseResult, TestRunSummary } from './report.ts'

export const name = 'tool-test-runner'
export const inject = ['tools']

/** Model-supplied description of one test case before execution. */
...
/** Maximum failing cases whose evidence the rendered tool result repeats. */
const MAX_RENDERED_FAILURES = 5
/** Maximum characters of one failing case's captured output kept in the rendered tool result. */
const RENDERED_OUTPUT_LIMIT = 1_000
...
/**
 * Keep a failing case's diagnosis for the rendered tool result. A failing
 * command prints its error last, so the tail carries the useful part.
 * @param text - the case's captured stdout followed by its stderr.
 * @returns the text unchanged, or a marker plus its last {@link RENDERED_OUTPUT_LIMIT} characters.
 */
function tailOutput(text: string): string {
  if (text.length <= RENDERED_OUTPUT_LIMIT) return text
  return TRUNCATION_MARKER.trim() + ' ' + text.slice(-RENDERED_OUTPUT_LIMIT)
}
 ...
 */
function validateTestCase(testCase: TestCaseInput): void {
 ...
 */
function runTestCase(testCase: TestCaseInput, signal: AbortSignal): Promise<TestCaseResult> {
  ...
  return new Promise((resolve, reject) => {
    execFile(
      'bash',
      ['-c', testCase.command],
      { encoding: 'utf8', signal, maxBuffer: 64 * 1024 * 1024, ...testCase.timeoutMs !== undefined ? { timeout: testCase.timeoutMs } : {} },
      (error, stdout, stderr) => {
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'run_tests',
    description:
      'Execute a list of shell-command test cases sequentially and write a detailed, self-contained HTML '
    ...
    output: {
      ...
      render: (_args, value) => {
        const lines = [
          `Ran ${value.summary.total} tests: ${value.summary.passed} passed, ${value.summary.failed} failed. `
          + `Report written to ${value.reportPath}.`,
        ]
        const failures = value.results.filter(result => !result.passed)
        for (const failure of failures.slice(0, MAX_RENDERED_FAILURES)) {
          const outcome = failure.exitCode === null ? 'did not exit normally' : `exited ${String(failure.exitCode)}`
          lines.push('', `Test case ${JSON.stringify(failure.name)} ${outcome}, expected ${String(failure.expectedExitCode)}.`)
          lines.push(`Command: ${failure.command}`)
          const evidence = (failure.stdout + failure.stderr).trim()
          if (evidence.length > 0) lines.push(tailOutput(evidence))
        }
        if (failures.length > MAX_RENDERED_FAILURES) {
          lines.push('', `${failures.length - MAX_RENDERED_FAILURES} further failing case(s) are in the report.`)
        }
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        throw new Error(`run_tests: failed to write report to ${args.reportPath}: ${(error as Error).message}`)
      }
