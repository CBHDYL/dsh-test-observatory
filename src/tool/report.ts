...
import { formatDuration } from '../command/runner.ts'
...
export function renderReport(title:string,summary:TestRunSummary,results:readonly TestCaseResult[]):string {
  ...
  const model:ReportModel={
    ...
    tests:results.map(result=>({ kind:'test' as const,name:result.name,path:result.command,status:result.passed?'passed':'failed',suite:'Command',durationSeconds:result.durationMs/1000,owner:'Unassigned',stdout:'Exit code: '+String(result.exitCode)+' (expected '+result.expectedExitCode+')\n\n'+result.stdout,stderr:result.stderr,...(result.passed?{}:{ error:'Exit '+String(result.exitCode)+'; expected '+result.expectedExitCode }) })),
  }
