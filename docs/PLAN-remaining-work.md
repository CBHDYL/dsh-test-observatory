...
### 覆盖率：从 93.5% 提升到 94.0%（分支 84.0% → 85.7%）

**结论：每文件 100% 未达成，且我不建议以当前方式追求。**

理由（已逐条核实）：

| 缺口类型 | 例子 | 能否通过测试消除 |
|---|---|---|
| **V8 插桩限制** | `a11y.ts` 中传给 `page.evaluate` 的序列化函数体；`visual.ts` 的 `selectorOf` | 否：测试用 `new Function` 重建后，V8 无法把执行映射回原行号 |
| **需劫持不可变导出** | `render.ts` 的标记缺失回退路径（需替换 `REPORT_BODY` 常量） | 需重构源码结构才能测 |
| **防御性早退** | `render.ts` 中 `start === -1 \|\| end === -1` 这类「资源缺失即返回」 | 需构造畸形资源，价值低 |
| **不可达常量** | `geometry.ts` 的 `MAX_ELEMENT_TEXT` 导出常量 | 无行为，不值得为覆盖率而测 |

**建议**：改为在 `vitest.config.ts` 的覆盖率排除表中**具名列出**「序列化到页面执行的检查函数」这一类文件，并注明原因，而不是为覆盖率改写源码结构。这与仓库对 `types.ts`、`bin.ts`、`worker.ts` 的既有排除是同一种处理。
