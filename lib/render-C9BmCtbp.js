...
function applyCharts(m){
  ...
  if(svg&&m.trend&&m.trend.length){
    ...
    var dots=pts.map(function(p,i){return '<circle class="chart-point" data-tip="'+p.run+' · Score '+p.score+' · '+p.durationSeconds+'s" cx="'+xs(i)+'" cy="'+ys(p.score)+'" r="4" fill="#635bff"/><text x="'+(xs(i)-18)+'" y="218">'+(n>8&&i%Math.ceil(n/8)!==0?'':(n>8?String(p.run).slice(-3):String(p.run).slice(-4)))+'</text>'}).join('');
...
function applyChecks(m){
  ...
  var hasNote=m.checks.some(function(c){return c.interpretation||c.nextAction});
  ...
  host.innerHTML='<div class="table-wrap"><table><thead>'+head+'</thead><tbody>'+m.checks.map(function(c){var where=(c.evidence||[]).map(function(e){return '<code>'+e.selector+'</code>'}).join('<br>')||'—';var note=c.interpretation?(c.interpretation+(c.nextAction?' <b>Next:</b> '+c.nextAction:'')):'—';return '<tr><td><span class="badge '+(c.severity==='high'?'failed':'flaky')+'">'+c.severity+'</span></td><td>'+c.family+'</td><td><b>'+c.rule+'</b></td><td>'+c.detail+'</td><td class="mono">'+where+'</td><td>'+c.persona+'</td>'+(hasNote?'<td>'+note+'</td>':'')+'</tr>'}).join('')+'</tbody></table></div>';
}
...
function fillDrawer(t){
  ...
  if(at)at.innerHTML=t.attachments.length?t.attachments.map(function(a){function esc(v){return String(v||'').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#39;')}var safePath=esc(a.path),safeName=esc(a.name||'Screenshot'),safeKind=esc(a.kind);var preview=a.kind==='screenshot'?'<img src="'+safePath+'" alt="'+safeName+'" style="width:100%;max-height:260px;object-fit:contain;border-radius:10px;background:var(--bg)">':'';return '<div class="attachment">'+preview+'<b>'+safeName+'</b><small>'+safeKind+' · '+safePath+'</small><a class="btn" href="'+safePath+'" target="_blank" download>Open / download</a></div>'}).join(''):'<div class="evidence-empty">No artifact is linked to this test.</div>';
}
