...
import { n as renderReport } from "./render-CwsFL6th.js";
import { r as runCase, t as buildReportModel } from "./runner-BIRflKyu.js";
import { _ as findDuplicateEvidence, f as runExperience, h as llmDecide, r as guidanceFor, s as scoreRun } from "./experience-B6KKI-ct.js";
import { mkdir, readFile, rename, rm, writeFile } from "node:fs/promises";
import { dirname, join, resolve } from "node:path";
import { parse } from "yaml";
import { execFile } from "node:child_process";
import { createHash } from "node:crypto";
//#region lib/types/command/types.js
/**
...
*/
const STRUCTURED_RESULT_FORMATS = [
	"junit",
	"vitest",
	"jest",
	"playwright",
...
function toStructuredResult(raw, where) {
	...
	if (!STRUCTURED_RESULT_FORMATS.includes(format)) throw new SuiteConfigError(`${where}.format: must be ${listFormats()}`);
	return {
		format,
...
function toReportOptions(raw) {
	...
	if (narrative !== void 0 && narrative !== false) {
		...
		options.narrative = {
			provider: requireString(route, "provider", "report.narrative"),
			model: requireString(route, "model", "report.narrative")
		};
	}
	return options;
}
...
function toJourney(raw, index) {
	...
	if (goal !== void 0 && (typeof goal !== "string" || goal.trim().length === 0)) throw new SuiteConfigError(`${where}.goal: must be a non-empty string`);
	const budget = optionalPositiveInt(record, "budget", where);
	const start = record["start"];
	if (start !== void 0 && (typeof start !== "string" || start.trim().length === 0)) throw new SuiteConfigError(`${where}.start: must be a non-empty URL`);
	if (goal !== void 0 && start === void 0) throw new SuiteConfigError(`${where}.start: a journey with a "goal" must declare where the user starts`);
	const rawSteps = record["steps"];
	if (goal !== void 0 && rawSteps !== void 0) throw new SuiteConfigError(`${where}: declare either "steps" or "goal", not both`);
	if (goal === void 0 && (!Array.isArray(rawSteps) || rawSteps.length === 0)) throw new SuiteConfigError(`${where}: "steps" must be a non-empty list, or the journey must declare a "goal"`);
	if (budget !== void 0 && goal === void 0) throw new SuiteConfigError(`${where}.budget: only an agent-driven journey with a "goal" takes a budget`);
...
//#endregion
//#region lib/types/experience/verdict.js
/**
...
*/
function decideJourneyVerdict(journey, findings) {
	const reasons = [];
...
*/
function decideRunVerdict(inputs) {
	const reasons = [];
	const limitations = [];
	...
	reasons.push(...limitations);
	if (high.length > 0 || inconclusive.length > 0 || failed.length > 0 || !inputs.coverageKnown || inputs.commit.length === 0) return {
...
function checkGuidance(finding) {
	...
	return {
		requirement,
		fix: known?.fix ?? "Follow the linked rule documentation for the elements listed below.",
...
function toExperienceSection(run) {
	...
	const verdictFindings = run.checks.flatMap((group) => [
		...group.visual.map((check) => ({
			severity: check.severity,
			family: "visual",
			persona: group.persona
		})),
		...group.accessibility.map((check) => ({
			severity: check.severity,
			family: "accessibility",
			persona: group.persona
		})),
		...group.keyboard.map((check) => ({
			severity: check.severity,
			family: "keyboard",
			persona: group.persona
		}))
	...
	const journeys = run.journeys.map((journey) => {
		const decision = decideJourneyVerdict({
			...
			agentDriven: journey.stopReason !== void 0,
		...
		return {
			...
			steps: journey.steps.map((step, index) => ({
				...
				...journey.trace?.[index] === void 0 ? {} : {
					reasoning: journey.trace[index].reasoning,
					result: journey.trace[index].result,
					changed: journey.trace[index].changed
				}
			})),
			...journey.stopReason === void 0 ? {} : { stopReason: journey.stopReason },
			...journey.obstacles === void 0 ? {} : { obstacles: journey.obstacles }
		};
	});
	const duplicateOf = new Map(findDuplicateEvidence(run.shots.map((shot) => ({
		id: shot.id,
		imageDataUri: shot.dataUri
	}))).map((entry) => [entry.id, entry.firstId]));
	const evidence = run.shots.map((shot) => ({
		...
		...duplicateOf.has(shot.id) ? { integrityDefects: [...shot.integrityDefects ?? [], {
			rule: "evidence-duplicate",
			detail: "this capture is byte-identical to " + String(duplicateOf.get(shot.id)) + ", so it is one observation shown twice"
		}] } : {}
	}));
	...
	for (const entry of run.checks) for (const [family, findings] of [
		...
		checks.push({
			...
			...finding.cropDataUri === void 0 ? {} : { cropDataUri: finding.cropDataUri }
		});
	}
	return {
		experience: score,
...
*/
function describeDetection(detection, directory, declared) {
	...
	const lines = ["Detected " + detection.projectType + " in " + directory + ":", ...detection.checks.map((check) => "  - " + check.name + "  (" + check.command + ")")];
	if (declared === void 0) lines.push("", "Paste this into test-observatory.yml, then run /test:");
	else lines.push("", declared.path + " already declares " + String(declared.cases) + " case(s); /test runs it as it stands.", "To replace it, paste this instead:");
...
//#endregion
//#region lib/types/command/git.js
/**
...
*/
async function detectRevision(cwd) {
	const [commit, branch] = await Promise.all([revParse(cwd, [
		"rev-parse",
		"--short",
		"HEAD"
	]), revParse(cwd, [
	...
	return {
		branch: branch === DETACHED_HEAD ? "" : branch,
		commit
	};
}
//#endregion
...
async function readStructuredResult(spec, testCase, cwd) {
	...
	const kind = spec.format === "sarif" ? "finding" : "test";
...
//#endregion
//#region lib/types/command/narrative.js
/**
...
*/
function llmNarrativeWriter(llm, route) {
...
*/
function buildNarrativePrompt(model) {
	...
	const digest = {
		...
		failingTests: executed.filter((test) => test.status === "failed").slice(0, 10).map((test) => ({
			name: test.name,
			path: test.path,
			suite: test.suite,
			framework: test.framework ?? "unknown",
			error: clip(test.error ?? "")
		})),
		openFindings: (model.checks ?? []).slice(0, 40).map((check) => ({
			rule: check.rule,
			family: check.family,
			severity: check.severity,
			observation: clip(check.detail),
			page: check.persona,
			elements: (check.evidence ?? []).map((element) => ({
				selector: element.selector,
				text: clip(element.text ?? "")
			}))
		})),
		experience: model.experience === void 0 ? null : {
			score: model.experience.total,
			tasksObserved: model.experience.tasksObserved,
			tasksCompleted: model.experience.tasksCompleted,
			dimensions: model.experience.dimensions.map((dimension) => ({
...
function prose(value, field) {
	if (typeof value !== "string" || value.trim().length === 0) throw new Error(`narrative ${field} must be a non-empty string`);
	return value.trim();
}
/**
...
*/
function parseRunNarrative(text) {
	const start = text.indexOf("{");
	const end = text.lastIndexOf("}");
	if (start < 0 || end <= start) throw new Error("narrative reply contains no JSON object");
	let parsed;
	try {
	...
	if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("narrative reply must be a JSON object");
	const record = parsed;
	const rawFindings = record["findings"];
	if (rawFindings !== void 0 && !Array.isArray(rawFindings)) throw new Error("narrative findings must be an array when present");
	const findings = [];
	for (const raw of Array.isArray(rawFindings) ? rawFindings : []) {
		...
		if (rule.length === 0 || interpretation.length === 0 || nextAction.length === 0) continue;
		findings.push({
			rule,
			interpretation,
			nextAction
...
//#endregion
//#region lib/types/report/selfcheck.js
/**
...
*/
function verifyReport(model) {
	const violations = [];
	const openFindings = model.tests.filter((row) => row.kind === "finding").filter((row) => row.status === "failed");
	if (model.summary.findings !== openFindings.length) violations.push({
	...
	for (const journey of model.journeys ?? []) if (journey.verdict === "PASS" && (journey.assertions ?? 0) === 0) violations.push({
	...
	if (high.length > 0 && model.verdict.label === "READY") violations.push({
		rule: "verdict-cleared-with-open-blockers",
		detail: "the verdict is READY while " + String(high.length) + " blocking finding(s) are open"
	});
	if (high.length > 0 && (model.verdict.reasons ?? []).every((reason) => !reason.includes("blocking"))) violations.push({
	...
	if ((model.checks ?? []).filter((check) => check.family === "keyboard" && check.severity === "high").length > 0) for (const journey of model.journeys ?? []) {
	...
	if ((model.meta.commit ?? "").length === 0 && (model.trend ?? []).length > 0) violations.push({
	...
	if (model.summary.coveragePercent === null && (model.verdict.reasons ?? []).every((reason) => !reason.includes("coverage"))) violations.push({
	...
	for (const shot of model.evidence ?? []) {
		if (shot.provenance === void 0 || shot.provenance.artifactHash.length === 0) violations.push({
			rule: "evidence-without-provenance",
			detail: "capture " + JSON.stringify(shot.id) + " carries no run, time or content hash to trace it by"
		});
		if (!(shot.imageDataUri ?? "").startsWith("data:image/")) violations.push({
...
async function annotate(ctx, model, config, signal) {
	...
	if (llm === void 0) return {
		model,
		note: " Model interpretation was requested but no llm service is mounted."
	};
	try {
		return {
			model: applyNarrative(model, parseRunNarrative(await llmNarrativeWriter(llm, route)({
				system: NARRATIVE_SYSTEM,
				prompt: buildNarrativePrompt(model),
				signal
			}))),
			note: ""
		};
		...
		return {
			model,
			note: " Model interpretation failed: " + (error instanceof Error ? error.message : String(error))
		};
	}
}
async function execute(invocation, ctx) {
	...
	if (argument === "auto") {
		...
		try {
			declared = {
				path: suitePath,
				cases: (await loadSuiteConfig(suitePath)).cases.length
			};
		} catch {
			declared = void 0;
		}
		return {
			kind: "success",
			text: describeDetection(detection, directory, declared)
		};
	}
	...
	const agentDecide = agentRoute === void 0 || agentLlm === void 0 ? void 0 : llmDecide(agentLlm, agentRoute);
	...
	const experienceSection = config.journeys === void 0 ? void 0 : toExperienceSection(await runExperience({
		...
		...agentDecide === void 0 ? {} : { agentDecide }
	}));
	...
	if (historyPath !== false) try {
		...
		const historyCounts = {
			passed: executed.filter((test) => test.status === "passed").length,
			failed: executed.filter((test) => test.status === "failed").length,
			skipped: executed.filter((test) => test.status === "skipped").length,
			flaky: executed.filter((test) => test.status === "flaky").length
		};
		...
		model = {
			...
			causes: historyCounts.failed > 0 ? [{
				...
				label: "Open static-analysis result",
				count: findings
			}] : [],
			slowest: [...executed].sort((left, right) => right.durationSeconds - left.durationSeconds).slice(0, 10).map((test, index) => ({
				rank: index + 1,
				name: test.name,
				suite: test.suite,
			...
			verdict: (() => {
				...
				const headline = historyCounts.failed > 0 ? historyCounts.failed + " tests failed." : findings > 0 ? "Every executed test passed; " + findings + " static-analysis result(s) need review." : retried > 0 ? retried + " test(s) passed only after a retry." : findings > 0 ? "Every executed test passed; " + findings + " static-analysis result(s) need review." : experienceRisk ? "Every executed test passed; the browser run recorded findings." : "Every test passed.";
	...
	const verdictInputs = {
		journeys: (experienceSection?.journeys ?? []).map((journey) => ({
			verdict: journey.verdict ?? "INCONCLUSIVE",
			persona: journey.personaId
		})),
		findings: (experienceSection?.checks ?? []).map((check) => ({
			severity: check.severity,
			family: check.family,
	...
	if (model.meta.commit.length === 0 && (model.trend.length > 0 || model.regressions.length > 0 || model.recovered.length > 0)) model = {
		...model,
		trend: [],
		regressions: [],
		recovered: []
	...
	model = {
		...
		verdict: {
			...model.verdict,
			label: runDecision.verdict,
			confidence: confidenceOf(verdictInputs),
	...
	const selfCheck = violations.length === 0 ? "" : " The report contradicts itself: " + violations.map((violation) => violation.rule).join(", ") + ".";
	return {
		kind: "success",
		text: `${model.summary.passed}/${model.summary.total} tests passed.${unstable}${findingLine} Report: ${outputPath}${annotated.note}${selfCheck}` + (failedTests.length === 0 ? "" : `\nFailed: ${failedTests.map((test) => test.name).join(", ")}`) + suiteNote
	};
}
//#endregion
