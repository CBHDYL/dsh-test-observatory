...
import { n as renderReport } from "./render-BZsZWLm5.js";
...
async function execute(invocation) {
	...
	return {
		kind: "success",
		text: `${model.summary.passed}/${model.summary.total} tests passed.${unstable}${findingLine} Report: ${outputPath}` + (failedTests.length === 0 ? "" : `\nFailed: ${failedTests.map((test) => test.name).join(", ")}`)
	};
}
//#endregion
