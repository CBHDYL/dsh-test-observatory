...
import { n as renderReport } from "./render-FmVxr96i.js";
	...
	if (historyPath !== false) try {
		...
		model = {
			...
			verdict: (() => {
				...
				const headline = historyCounts.failed > 0 ? historyCounts.failed + " tests failed." : findings > 0 ? "Every executed test passed; " + findings + " scan finding(s) need review." : retried > 0 ? retried + " test(s) passed only after a retry." : findings > 0 ? "Every executed test passed; " + findings + " scan finding(s) need review." : experienceRisk ? "Every executed test passed; the browser run recorded findings." : "Every test passed.";
