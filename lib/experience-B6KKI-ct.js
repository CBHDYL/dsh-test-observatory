	...
	const inspect = (config) => {
		...
		if (withoutIndicator.length > 0) {
			...
			found.push({
				...
				severity: withoutShare >= .5 ? "high" : "medium",
...
async function runJourney(page, spec, capture, retries, settleTimeoutMs, setActiveStep, behavior, appliedEnvironment) {
	...
	const assertions = (spec.steps ?? []).reduce((count, step, index) => steps[index]?.state !== "PASS" ? count : count + step.actions.filter((action) => action.kind === "expectText" || action.kind === "expectVisible").length, 0);
	return {
		persona: spec.persona,
		device: spec.device,
		name: spec.name,
