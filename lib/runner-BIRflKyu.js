...
function buildReportModel(outcomes, inputs) {
	...
	const headline = failed > 0 ? `${failed} of ${total} tests failed.` : findings > 0 ? `Every executed test passed; ${findings} static-analysis result${findings === 1 ? "" : "s"} need review.` : "Every declared test passed.";
	const verdictLabel = failed > 0 ? "Suite failing" : findings > 0 ? "Suite passing · findings open" : "Suite passing";
	const verdictSummary = failed > 0 ? "At least one declared command did not produce its expected exit code." : findings > 0 ? "The executed tests completed without failures. The static-analysis results below are unresolved and were not executed as tests." : "The declared command suite completed without failures.";
	const verdictRisk = failed > 0 ? "Failing commands are reported with their captured output; inspect the details table before release." : findings > 0 ? "No test failed. Resolve or waive the open static-analysis results before release." : "No failing command in this run.";
	const model = {
		meta: {
			project,
		...
		kpis: [
			{
				label: "Pass rate",
				value: `${passRate}%`,
				delta: `${passed} of ${total} executed`,
				...failed > 0 ? { worse: true } : {}
			},
			{
		...
		summary: {
			...
			...inputs.snapshots === void 0 ? {} : { snapshots: inputs.snapshots }
		},
		trend: [],
		causes: failed > 0 ? [{
			label: "Non-zero exit",
