...
function buildReportModel(outcomes, inputs) {
	...
	const headline = failed > 0 ? `${failed} of ${total} tests failed.` : findings > 0 ? `Every executed test passed; ${findings} scan ${findings === 1 ? "finding" : "findings"} need review.` : "Every declared test passed.";
	const verdictLabel = failed > 0 ? "Suite failing" : findings > 0 ? "Suite passing · findings open" : "Suite passing";
	const verdictSummary = failed > 0 ? "At least one declared command did not produce its expected exit code." : findings > 0 ? "The executed tests completed without failures. The scan findings below are unresolved and were not executed as tests." : "The declared command suite completed without failures.";
	const verdictRisk = failed > 0 ? "Failing commands are reported with their captured output; inspect the details table before release." : findings > 0 ? "No test failed. Resolve or waive the open scan findings before release." : "No failing command in this run.";
	const model = {
		meta: {
			project,
