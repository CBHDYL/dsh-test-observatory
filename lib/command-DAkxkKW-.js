...
import { n as renderReport } from "./render-j4Jxz-IZ.js";
...
async function execute(invocation, ctx) {
	...
	return {
		kind: "success",
		text: `${model.summary.passed}/${model.summary.total} tests passed.${unstable}${findingLine} Report: ${outputPath}${annotated.note}` + (failedTests.length === 0 ? "" : `\nFailed: ${failedTests.map((test) => test.name).join(", ")}`)
	};
}
//#endregion
