...
import { n as renderReport } from "./render-C5daeIno.js";
import { r as runCase, t as buildReportModel } from "./runner-d41y-OKe.js";
	...
	if (historyPath !== false) try {
		...
		model = {
			...
			verdict: (() => {
				...
				const headline = historyCounts.failed > 0 ? historyCounts.failed + " tests failed." : findings > 0 ? "Every executed test passed; " + findings + " scan finding(s) need review." : retried > 0 ? retried + " test(s) passed only after a retry." : findings > 0 ? "Every executed test passed; " + findings + " scan finding(s) need review." : experienceRisk ? "Automated tests passed; experience checks scored " + experienceScore + "/100." : "Every test passed.";
