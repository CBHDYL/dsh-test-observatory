...
import { n as renderReport } from "./render-Di7fX9zC.js";
import { a as scoreRun, d as runExperience } from "./experience-BgmacqXy.js";
	...
	if (inputs.experienceSection !== void 0) {
		...
		return {
			...
			verdict: {
				...model.verdict,
				score: combinedScore,
				headline: combinedScore === 100 ? model.verdict.headline : `Automated tests passed; experience checks scored ${experienceScore}/100.`,
				label: combinedScore === 100 ? model.verdict.label : "Review experience findings",
				summary: combinedScore === 100 ? model.verdict.summary : "The test suite passed, but browser observations found release risks.",
...
function parseJUnit(source) {
	...
	for (const match of source.matchAll(/<testcase\b([^>]*?)(?:\/>|>([\s\S]*?)<\/testcase>)/g)) {
		...
		rows.push({
			name: decode(attr(attrs, "name") ?? "unnamed test"),
			status: failure ? "failed" : /<skipped\b/.test(body) ? "skipped" : "passed",
			...path ? { path } : {},
			...suite ? { suite } : {},
			...Number.isFinite(durationSeconds) ? { durationSeconds } : {},
			...failure ? { error: decode(failure[3]?.replace(/<!\[CDATA\[|\]\]>/g, "").trim() || attr(failure[2] ?? "", "message") || "test failed") } : {}
		});
	}
	return rows;
}
...
async function parseStructuredResult(spec, testCase, cwd) {
	const source = await readFile(resolve(cwd, spec.path), "utf8");
	const parsed = spec.format === "junit" || spec.format === "pytest" ? parseJUnit(source) : (() => {
	...
	return parsed.map((item) => ({
		name: item.name,
		path: item.path ?? (spec.format === "pytest" && item.suite ? item.suite.replaceAll(".", "/") + ".py" : spec.path),
		status: item.status,
...
async function projectHistory(path, model) {
	...
	const tests = model.tests.map((test) => {
		...
		if (old?.status === "failed" && test.status === "passed") recovered.push({
			name: test.name,
			evidence: "Passed after failing in " + previous?.runId
		});
		const statuses = history.runs.slice(-4).flatMap((run) => run.tests.filter((item) => key(item) === key(test)).map((item) => item.status));
		return test.status === "passed" && statuses.includes("passed") && statuses.includes("failed") ? {
			...test,
			status: "flaky"
		} : test;
	});
	...
	if (historyPath !== false) try {
		...
		model = {
			...
			verdict: (() => {
				...
				const headline = historyCounts.failed > 0 ? historyCounts.failed + " tests failed." : historyCounts.flaky > 0 ? historyCounts.flaky + " tests are flaky." : experienceRisk ? "Automated tests passed; experience checks scored " + experienceScore + "/100." : "Every test passed.";
			...
			kpis: [
				...
				{
					label: "Needs review",
					value: String(historyCounts.failed + historyCounts.flaky),
					delta: historyCounts.flaky ? historyCounts.flaky + " flaky" : historyCounts.failed ? historyCounts.failed + " failed" : "none",
					...hasRisk ? { worse: true } : {}
				}
			]
		};
