...
import { r as renderReport } from "./report-BQ4_76FK.js";
	...
	if (historyPath !== false) try {
		...
		model = {
			...
			verdict: {
				...
				headline: historyCounts.failed > 0 ? historyCounts.failed + " tests failed." : historyCounts.flaky > 0 ? historyCounts.flaky + " tests are flaky." : "Every test passed.",
				label: hasRisk ? "Suite needs review" : "Suite passing",
				summary: hasRisk ? "Review failed and flaky tests before release." : "The test suite completed without failures.",
				confidence: passRate + "% stable pass rate",
				risk: hasRisk ? "Historical comparison found unstable or failing tests." : "No failing or flaky test in this run."
			},
			kpis: [
				{
					label: "Pass rate",
