 ...
 */
export function evidenceOf(node, measured) {
    ...
    const box = measured ?? { x: 0, y: 0, width: 0, height: 0, space: 'viewport' };
    return { element, box };
}
/**
...
export async function checkAccessibility(page) {
    ...
    const selectors = report.violations.flatMap(violation => violation.nodes.map(node => selectorOf(node))).filter((selector) => selector !== undefined);
    const measured = await page.evaluate((wanted) => wanted.map((selector) => {
        try {
            ...
            if (rect.width === 0 && rect.height === 0)
                return null;
            return { x: Math.round(rect.x), y: Math.round(rect.y), width: Math.round(rect.width), height: Math.round(rect.height), space: 'viewport' };
        }
        catch {
    ...
    const boxBySelector = new Map();
    selectors.forEach((selector, index) => { const box = measured[index]; if (box !== null && box !== undefined)
        boxBySelector.set(selector, box); });
    return report.violations.map((violation) => {
