 ...
 */
export function buildTurnPrompt(goal, observation, trace) {
 ...
 */
export function parseDecision(text, elementCount) {
    const start = text.indexOf('{');
    const end = text.lastIndexOf('}');
    if (start < 0 || end <= start)
        throw new Error('agent reply contains no JSON object');
    let parsed;
    try {
    ...
    if (reasoning.length === 0)
        throw new Error('agent reply must state its reasoning');
...
function describe(action, observation) {
    if (action.kind === 'back')
        return 'go back';
    if (action.kind === 'done')
        return 'finish the journey';
    if (action.kind === 'stuck')
        return 'report an obstacle';
...
async function perform(page, action, observation) {
    ...
    if (action.kind === 'done')
        return 'finished';
    if (action.kind === 'stuck')
        return 'reported an obstacle';
    const element = observation.elements.find(entry => entry.index === action.index);
    if (element === undefined)
        return 'the element was no longer present';
    const selector = 'a, button, input, select, textarea, [role="button"], [role="link"], [role="tab"], [tabindex]';
    ...
    try {
        if (action.kind === 'click')
            ...
            await target.fill(action.text, { timeout: 5_000 });
 ...
 */
export async function runAgent(page, goal, decide, options = {}) {
    ...
    for (let turn = 1; turn <= budget; turn += 1) {
        ...
        const entry = { turn, reasoning: decision.reasoning, action: describe(decision.action, observation), result, changed };
        trace.push(entry);
 ...
 */
export function llmDecide(llm, route) {
