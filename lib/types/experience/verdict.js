 ...
 */
export function decideJourneyVerdict(journey, findings) {
    const reasons = [];
 ...
 */
export function decideRunVerdict(inputs) {
    const reasons = [];
    ...
    const limitations = [];
    if (!inputs.coverageKnown)
        limitations.push('coverage was not measured, so untested code is unknown');
    if (inputs.commit.length === 0)
        limitations.push('the run recorded no commit, so it cannot be compared with a baseline');
    ...
    if (high.length > 0)
        reasons.push(String(high.length) + ' open blocking finding(s) that no test covers');
    const inconclusive = inputs.journeys.filter(journey => journey.verdict === 'INCONCLUSIVE');
    if (inconclusive.length > 0)
        reasons.push(String(inconclusive.length) + ' journey(s) settled without establishing anything: ' + inconclusive.map(journey => journey.persona).join(', '));
    const failed = inputs.journeys.filter(journey => journey.verdict === 'FAIL');
    if (failed.length > 0)
        reasons.push(String(failed.length) + ' journey(s) failed: ' + failed.map(journey => journey.persona).join(', '));
    reasons.push(...limitations);
 ...
 */
export function confidenceOf(inputs) {
