...
import { checkKeyboard, probeOpenDialog } from "./keyboard-checks.js";
import { applyEnvironment } from "./behavior/environment.js";
import { behaviorDimensions, resolveBehavior } from "./behavior/index.js";
import { MAX_SHOT_BYTES as MAX_SHOT_BYTES_LIMIT, captureElement, captureEvidence, captureFindingCrops, captureFocused } from "./capture.js";
...
async function runAction(page, action, capture, settleTimeoutMs) {
    switch (action.kind) {
        case 'goto':
            ...
            //
            // The policy's settle budget bounds the navigation itself, so a persona
            // that declares how long it waits actually waits that long: a slow profile
            // under the browser's own default would otherwise fail before the budget
            // the report claims was applied.
            await page.goto(action.url, { waitUntil: 'load', timeout: settleTimeoutMs });
...
async function runGoalJourney(page, spec, decide, capture, behavior, appliedEnvironment) {
    ...
    const steps = run.trace.map((entry) => {
        ...
        const state = entry.action === 'finish the journey'
            ? 'PASS'
