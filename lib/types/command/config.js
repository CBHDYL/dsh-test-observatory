...
import { STRUCTURED_RESULT_FORMATS } from "./types.js";
...
function toStructuredResult(raw, where) {
    ...
    if (!STRUCTURED_RESULT_FORMATS.includes(format)) {
    ...
    return { format: format, path: requireString(record, 'path', where) };
}
/**
...
function toReportOptions(raw) {
    ...
    if (historyPath !== undefined) {
        if (historyPath !== false && (typeof historyPath !== 'string' || historyPath.trim().length === 0))
            throw new SuiteConfigError('report.historyPath: must be a non-empty string or false');
    ...
    if (narrative !== undefined && narrative !== false) {
        ...
        if (route === null)
            throw new SuiteConfigError('report.narrative: must be a mapping with provider and model, or false');
...
function toBehavior(raw, where) {
    if (typeof raw === 'string') {
        if (raw.trim().length === 0)
            throw new SuiteConfigError(`${where}.behavior: must be a non-empty string`);
        return raw;
    }
    const record = asRecord(raw);
    if (record === null)
        throw new SuiteConfigError(`${where}.behavior: must be a preset name or a mapping`);
    const preset = requireString(record, 'preset', `${where}.behavior`);
    const override = { preset };
    const sections = ['input', 'timing', 'modality', 'environment', 'recovery'];
    for (const section of sections) {
        ...
        if (mapping === null)
            throw new SuiteConfigError(`${where}.behavior.${section}: must be a mapping`);
