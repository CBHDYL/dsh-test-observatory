...
export function buildReportModel(outcomes, inputs) {
    ...
    const headline = failed > 0
        ...
        : findings > 0
            ? `Every executed test passed; ${findings} static-analysis result${findings === 1 ? '' : 's'} need review.`
    ...
    const verdictSummary = failed > 0
        ...
        : findings > 0
            ? 'The executed tests completed without failures. The static-analysis results below are unresolved and were not executed as tests.'
    ...
    const verdictRisk = failed > 0
        ...
        : findings > 0
            ? 'No test failed. Resolve or waive the open static-analysis results before release.'
    ...
    const model = {
        ...
        causes: failed > 0
            ...
            : findings > 0
                ? [{ label: 'Open static-analysis result', count: findings }]
                : [],
        slowest: [...tests]
