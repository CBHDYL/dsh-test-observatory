...
export async function detectProject(directory) {
    ...
    if (node !== undefined)
        return { projectType: node.name, checks: nodeChecks(node.scripts) };
    const pyproject = await readOptional(join(directory, 'pyproject.toml'));
    if (pyproject !== undefined) {
        const checks = [{ name: 'pytest', command: 'python3 -m pytest', suite: 'Unit' }];
 ...
 */
export function describeDetection(detection, directory, declared) {
    ...
    if (declared === undefined) {
        ...
        lines.push('', declared.path + ' already declares ' + String(declared.cases) + ' case(s); /test runs it as it stands.', 'To replace it, paste this instead:');
    }
