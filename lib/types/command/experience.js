...
import { decideJourneyVerdict } from "../experience/verdict.js";
/**
...
export function toExperienceSection(run) {
    ...
    const verdictFindings = run.checks.flatMap(group => [
        ...group.visual.map(check => ({ severity: check.severity, family: 'visual', persona: group.persona })),
        ...group.accessibility.map(check => ({ severity: check.severity, family: 'accessibility', persona: group.persona })),
        ...group.keyboard.map(check => ({ severity: check.severity, family: 'keyboard', persona: group.persona })),
    ]);
