...
import { parseJUnitDocument } from "./junit.js";
import { parseSarif } from "./sarif.js";
import { readSnapshotCounts } from "./snapshots.js";
 ...
 */
export async function readStructuredResult(spec, testCase, cwd) {
    ...
    const parsed = spec.format === 'junit' || spec.format === 'pytest'
        ...
        : (() => {
            ...
            if (spec.format === 'api')
                return parseApi(value);
            if (spec.format === 'performance')
                return parsePerformance(value);
            if (spec.format === 'sarif')
                return parseSarif(value, cwd).map(finding => ({
            ...
            const out = [];
            if (spec.format === 'playwright')
                parsePlaywright(value, out);
            else
                visit(value, spec.format, out);
            return out;
        })();
    if (parsed.length === 0)
