...
export function renderReport(title, summary, results) {
    ...
    const model = {
        ...
        tests: results.map(result => ({ name: result.name, path: result.command, status: result.passed ? 'passed' : 'failed', suite: 'Command', durationSeconds: result.durationMs / 1000, owner: 'Unassigned', stdout: 'Exit code: ' + String(result.exitCode) + ' (expected ' + result.expectedExitCode + ')\n\n' + result.stdout, stderr: result.stderr, ...(result.passed ? {} : { error: 'Exit ' + String(result.exitCode) + '; expected ' + result.expectedExitCode }) }))
    };
