...
import { renderReport as renderObservatory } from "../report/render.js";
import { formatDuration } from "../command/runner.js";
/**
 ...
 */
export function renderReport(title, summary, results) {
    ...
    const model = {
        ...
        kpis: [{ label: 'Pass rate', value: passRate + '%', delta: summary.passed + ' of ' + summary.total, ...(summary.failed ? { worse: true } : {}) }, { label: 'Total tests', value: String(summary.total), delta: summary.total + ' observed' }, { label: 'Duration', value: formatDuration(durationSeconds), delta: 'wall clock' }, { label: 'Failed', value: String(summary.failed), delta: summary.failed ? 'needs review' : 'none', ...(summary.failed ? { worse: true } : {}) }],
        summary: { total: summary.total, passed: summary.passed, failed: summary.failed, findings: 0, skipped: 0, flaky: 0, durationSeconds, coveragePercent: null }, trend: [], causes: summary.failed ? [{ label: 'Non-zero exit', count: summary.failed }] : [],
        ...
        tests: results.map(result => ({ kind: 'test', name: result.name, path: result.command, status: result.passed ? 'passed' : 'failed', suite: 'Command', durationSeconds: result.durationMs / 1000, owner: 'Unassigned', stdout: 'Exit code: ' + String(result.exitCode) + ' (expected ' + result.expectedExitCode + ')\n\n' + result.stdout, stderr: result.stderr, ...(result.passed ? {} : { error: 'Exit ' + String(result.exitCode) + '; expected ' + result.expectedExitCode }) })),
    };
