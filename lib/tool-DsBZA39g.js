...
import { n as renderReport$1 } from "./render-CfZn7AI4.js";
import { n as formatDuration } from "./runner-BIRflKyu.js";
...
function renderReport(title, summary, results) {
	...
	const model = {
		...
		tests: results.map((result) => ({
			...
			durationSeconds: result.durationMs / 1e3,
			owner: "Unassigned",
			stdout: "Exit code: " + String(result.exitCode) + " (expected " + result.expectedExitCode + ")\n\n" + result.stdout,
...
const RENDERED_OUTPUT_LIMIT = 1e3;
/**
...
function tailOutput(text) {
	...
	return TRUNCATION_MARKER.trim() + " " + text.slice(-1e3);
}
/**
...
function apply(ctx) {
	ctx.tools.register(defineTool({
		...
		output: {
			...
			render: (_args, value) => {
				const lines = [`Ran ${value.summary.total} tests: ${value.summary.passed} passed, ${value.summary.failed} failed. Report written to ${value.reportPath}.`];
				...
				if (failures.length > MAX_RENDERED_FAILURES) lines.push("", `${failures.length - MAX_RENDERED_FAILURES} further failing case(s) are in the report.`);
				return [{
					type: "text",
					text: lines.join("\n")
				}];
